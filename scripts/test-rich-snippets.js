...
function generateTestUrls() {
  ...
  PAGES_TO_TEST.forEach(page => {
    ...
    Object.entries(VALIDATORS).forEach(([name, validatorUrl]) => {
      const testUrl = name === 'google' ? 
        `${validatorUrl}?url=${encodeURIComponent(fullUrl)}` :
        name === 'schema' ?
        `${validatorUrl}#url=${encodeURIComponent(fullUrl)}` :
        `${validatorUrl}?url=${encodeURIComponent(fullUrl)}`;
        
      console.log(`${name.charAt(0).toUpperCase() + name.slice(1)}: ${testUrl}`);
    });
  });
...
function checkStructuredDataFiles() {
  ...
  pages.forEach(page => {
    ...
    if (fs.existsSync(filePath)) {
      ...
      console.log(`${hasStructuredData ? '✅' : '❌'} ${page} ${hasStructuredData ? 'has' : 'missing'} structured data`);
    } else {
...
`);
  ...
  console.log('\n' + '═'.repeat(60));
  console.log('✨ Testing complete! Use the URLs above to validate your Rich Snippets implementation.');
  console.log('═'.repeat(60));
}
