...
export const careerSchema = Joi.object({
  ...
  clientFacingExperience: Joi.string()
    .min(10)
    .max(1000)
    .required()
    .messages({
      'string.min': 'Client-facing experience must be at least 10 characters long',
...
export function validateInput<T>(schema: Joi.ObjectSchema<T>, data: unknown): {
  error?: string;
  value?: T;
} {
