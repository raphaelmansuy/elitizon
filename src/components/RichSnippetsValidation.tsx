...
export default function RichSnippetsValidation({ schemas, pageUrl }: RichSnippetsValidationProps) {
  ...
  const validateSchema = (schema: StructuredDataSchema) => {
    ...
    if (schema['@type'] === 'Service' || schema['@type'] === 'ProfessionalService') {
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 pt-2 border-t border-slate-600 text-xs text-slate-300">
        Test with:
        <br />
        • <a href={`https://search.google.com/test/rich-results?url=${encodeURIComponent(pageUrl)}`} target="_blank" rel="noopener noreferrer" className="text-blue-300 hover:underline">Google Rich Results Test</a>
        <br />
        • <a href={`https://validator.schema.org/#url=${encodeURIComponent(pageUrl)}`} target="_blank" rel="noopener noreferrer" className="text-blue-300 hover:underline">Schema.org Validator</a>
      </div>
    </div>
  );
}
...
export function useRichSnippetsValidation(pageUrl: string) {
  const [schemas, setSchemas] = React.useState<Record<string, StructuredDataSchema>>({});
  ...
  return { 
    schemas, 
    RichSnippetsValidation: (props: Omit<RichSnippetsValidationProps, 'schemas' | 'pageUrl'>) => 
